...
# Implementation Details`;
    ...
    const postDataString = `name=${encodeURIComponent(postData.name)
    }&desc=${encodeURIComponent(postData.desc)
    }&idList=${encodeURIComponent(postData.idList)
    }&urlSource=null&due=null`;

    const url = `https://api.trello.com/1/cards?key=${window.config.trelloAppKey}&token=${window.config.trelloUserToken}`;
